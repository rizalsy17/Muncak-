...
export default function JoinRequestModal({
  ...
  const { loading, error, success, sendJoinRequest, hasRequested } =
    useJoinRequest(planningId, userId);
          ...
          {!loading && !error && !success && (
            <>
              <span className="text-darkText font-normal">
                {alreadyRequested
                  ? "You have already requested to join this plan."
                  : "Are you sure you want to request to join this plan?"}
              </span>
              {alreadyRequested && (
                <p className="text-green-500 font-normal">Already Requested</p>
              )}
